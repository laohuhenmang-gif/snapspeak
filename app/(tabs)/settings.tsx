...
import { getApiKey, setApiKey, clearApiKey, getModel, setModel, getProvider, setProvider, getBaseUrl, setCustomBaseUrl, getPersona, setPersona } from '../../services/ai-config';
...
export default function SettingsScreen() {
  ...
  useEffect(() => {
    ...
    getPersona().then(p => setPersonaState(p));
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.background }]} contentContainerStyle={{ padding: 12 }}>
      ...
      {/* AI Persona */}
      <PixelCard style={{ marginBottom: 12 }}>
        <Text style={[styles.sectionTitle, { color: theme.text }]}>AI 角色</Text>
        <View style={styles.row}>
          {PERSONAS.map(p => (
            <TouchableOpacity
              key={p.id}
              onPress={async () => { setPersonaState(p.id); await setPersona(p.id); }}
              style={[styles.option, { borderColor: theme.pixelBorder, backgroundColor: persona === p.id ? theme.primary : theme.surface }]}
            >
              <Text style={{ color: persona === p.id ? '#fff' : theme.text, fontFamily: 'monospace', fontSize: 11, fontWeight: '600' }}>
                {p.avatar} {p.name}
              </Text>
            </TouchableOpacity>
          ))}
        </View>
        <Text style={{ color: theme.textMuted, fontFamily: 'monospace', fontSize: 11, marginTop: 6 }}>
      ...
      {/* Security */}
      <PixelCard style={{ marginBottom: 12 }}>
        <Text style={[styles.sectionTitle, { color: theme.text }]}>安全</Text>
        <View style={styles.row}>
          <TouchableOpacity
            onPress={handleToggleAppLock}
            style={[styles.option, { borderColor: theme.pixelBorder, backgroundColor: appLockEnabled ? theme.primary : theme.surface }]}
          >
            <Text style={{ color: appLockEnabled ? '#fff' : theme.text, fontFamily: 'monospace', fontSize: 11, fontWeight: '600' }}>
              🔒 应用锁: {appLockEnabled ? '已开启' : '未开启'}
            </Text>
          </TouchableOpacity>
        </View>
      </PixelCard>
      ...
      {/* Data Retention */}
      <PixelCard style={{ marginBottom: 12 }}>
        <Text style={[styles.sectionTitle, { color: theme.text }]}>数据保留</Text>
        <Text style={{ color: theme.textMuted, fontFamily: 'monospace', fontSize: 11, marginBottom: 8 }}>
          已完成任务保留天数：{retentionDays} 天
        </Text>
        <View style={styles.row}>
          {[30, 60, 90, 180].map(d => (
            <TouchableOpacity
              key={d}
              onPress={() => handleChangeRetention(d)}
              style={[styles.option, { borderColor: theme.pixelBorder, backgroundColor: retentionDays === d ? theme.primary : theme.surface }]}
            >
              <Text style={{ color: retentionDays === d ? '#fff' : theme.text, fontFamily: 'monospace', fontSize: 11, fontWeight: '600' }}>
                {d}天
              </Text>
            </TouchableOpacity>
          ))}
