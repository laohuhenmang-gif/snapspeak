...
export default function HomeScreen() {
  ...
  const [contextMenu, setContextMenu] = useState<{ visible: boolean; content: string }>({ visible: false, content: '' });
  ...
  const handleCameraPress = useCallback(async () => {
    ...
    try {
      ...
      const compressed = await compressImage(asset.uri, asset.base64);

      // OCR
      const ocrResult = await recognizeText(compressed.base64 || asset.base64);
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <MessageContextMenu
        ...
        onClose={() => setContextMenu({ visible: false, content: '' })}
        onCopy={() => {
          try { require('expo-clipboard').setStringAsync(contextMenu.content); } catch {}
        }}
        onForward={() => handleSend(`帮我把这句话变成任务：${contextMenu.content}`)}
      />
    </View>
  );
}
