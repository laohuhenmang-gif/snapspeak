import { View, Text, FlatList, StyleSheet, AppState, KeyboardAvoidingView, Platform, Alert, Keyboard } from 'react-native';
...
export default function TodayScreen() {
  ...
  // Load greeting and today's tasks on focus (only once)
  useFocusEffect(
    useCallback(() => {
  ...
  const loadInitialMessages = async () => {
    ...
    try {
      ...
      const yesterdayPlanned = yesterdayTasks;
    ...
    scrollToBottom();
      ...
      const fallback: ChatMessage = {
        ...
        role: 'system',
        kind: 'error',
        content: `初始化失败: ${e?.message || '请检查网络和 API 配置后重试'}`,
        timestamp: Date.now(),
        errorRetryable: true,
      };
  ...
  const handleVoiceResult = useCallback(
    (text: string) => {
      handleSendText(text).catch(console.warn);
    },
    [handleSendText]
  );
  ...
  const handleCardAction = useCallback(
    async (actionKey: string, message: ChatMessage) => {
      ...
      if (actionKey === 'edit') {
        const originalContent = message.cardData?.items?.[0]?.value || message.content;
        Alert.alert('修改内容', `原内容：${originalContent}\n\n请在下方的输入框中输入修改后的内容。\n例如：\n"改为下午4点"\n"修改为明天上午10点"`);
        const editMsg: ChatMessage = {
          id: generateId(),
          role: 'assistant',
          kind: 'text',
          content: `好的，请修改「${originalContent}」，直接在输入框中输入新内容。`,
          timestamp: Date.now(),
        };
