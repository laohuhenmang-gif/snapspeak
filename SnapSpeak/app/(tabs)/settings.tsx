...
export default function SettingsScreen() {
  ...
  const handleSaveKey = async () => {
    ...
    try {
      ...
      Alert.alert('保存失败', e?.message || '无法保存 API Key，请重试');
    }
  };
  ...
  const handleSaveCustomUrl = async () => {
    ...
    try {
      ...
      Alert.alert('保存失败', e?.message || '无法保存 API 地址，请重试');
    }
  };
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      <TouchableOpacity style={[styles.card, { backgroundColor: theme.card, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }]}
        ...
        <View>
          <Text style={[styles.sectionTitle, { color: theme.text }]}>🔒 隐私政策</Text>
          <Text style={[styles.value, { color: theme.textLight }]}>了解数据如何被保护</Text>
        </View>
        <Text style={[styles.value, { color: theme.textLight }]}>→</Text>
      </TouchableOpacity>
