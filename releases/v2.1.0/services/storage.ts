...
function getDb(): SQLite.SQLiteDatabase {
  if (!db) {
    try {
      ...
      throw new Error('数据库打开失败: ' + (e instanceof Error ? e.message : String(e)));
    }
  }
  if (!db) {
...
export async function saveAIAction(action: {
  ...
  getDb().runSync(
    ...
    [id, action.action_type, action.payload_json || '{}', action.status || 'pending', now],
  );
  return id;
}
...
const ALLOWED_COLUMNS: Record<string, Set<string>> = {
  tasks: new Set(['id', 'title', 'description', 'planned_at', 'due_at', 'reminder_at', 'priority', 'category', 'recurring', 'completed', 'completed_at', 'source_type', 'source_id', 'project_name', 'status', 'postponed_count', 'current_blocker_reason', 'needs_precheck', 'created_at', 'updated_at']),
  captures: new Set(['id', 'type', 'raw_text', 'image_uri', 'audio_uri', 'transcribed_text', 'ocr_text', 'processed_status', 'created_at']),
  reminders: new Set(['id', 'task_id', 'remind_at', 'remind_type', 'status', 'notification_id', 'created_at']),
  ai_actions: new Set(['id', 'action_type', 'payload_json', 'status', 'user_confirmed_at', 'executed_at', 'error_message', 'created_at']),
  memories: new Set(['id', 'memory_type', 'content', 'source_id', 'confidence', 'user_confirmed', 'enabled', 'created_at', 'updated_at']),
  reflections: new Set(['id', 'date', 'planned_count', 'completed_count', 'completion_rate', 'overdue_count', 'main_blockers_json', 'ai_summary', 'ai_suggestion', 'created_at']),
  blockers: new Set(['id', 'task_id', 'blocker_type', 'blocker_text', 'suggested_action', 'next_remind_at', 'resolved', 'created_at']),
  conversations: new Set(['id', 'role', 'content', 'message_type', 'related_task_id', 'related_action_id', 'created_at']),
};
  ...
  try {
    ...
    for (const table of tables) {
      ...
      for (const row of rows) {
        try {
          const columns = Object.keys(row).filter((col) => allowed.has(col));
    ...
    db.execSync('ROLLBACK');
    errors.push(`事务回滚: ${e?.message || 'unknown'}`);
  }
